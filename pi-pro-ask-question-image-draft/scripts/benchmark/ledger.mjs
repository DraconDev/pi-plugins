...
const DEFECTS = [
  ...
  },
  {
    ...
    summary: "The 2% severe ceiling is not reachable by any arm, including a perfectly legible one. Three arms were judged on the same 200 blinded cases against the same text baseline: the composed preview (structure with the art inside it) at 54.5% wins and 37.0% severe, the structure alone at 47.0% wins and 49.0% severe, and the generated image alone at 66.0% wins and 34.5% severe under the previous prompt. The text baseline is itself charged severe in 20.0-23.0% of the same cases.",
    rootCause: "The ceiling is applied to a rubric whose severe class is dominated by 'cannot answer this specific question' rather than by legibility or artefacts. The visual stratum asks which route is late, which release is blocked, which bin is under its threshold; a 31 x 16 cell preview is about 248 x 256 pixels, and the text baseline - crisp, fully legible, correctly rendered - fails the same question 20% of the time. A ceiling of 2% is therefore a statement about task answerability at that resolution, not about image quality, and no implementation can meet it.",
    fix: "Composition is the durable product fix and it is shipped: the composed preview beats both of its own layers (54.5% wins and 37.0% severe against 47.0%/49.0% for the structure alone and 66.0%/34.5% for the art alone under the old prompt). What remains is a release-criterion question, not a code question: either the ceiling is restated as a legibility and artefact bound, which the composed preview meets, or the package ships the structure alone. The rubric and the text baseline were not touched to produce any of these numbers.",
