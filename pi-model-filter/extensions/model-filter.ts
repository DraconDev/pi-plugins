 ...
 *
 *   `filterModelsForProvider` filters a static model array;
 *   `wrapRefreshModels` wraps a `refreshModels` callback so its results
 *   are filtered live on every call (config changes apply without a
 *   /reload).
 *
...
// ─── Pure core (no pi or fs dependency — safe to import anywhere) ───────
