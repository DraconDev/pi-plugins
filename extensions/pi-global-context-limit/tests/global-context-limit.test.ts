...
function runtimeModel(id: string, contextWindow: number, maxTokens: number) {
  return {
    id,
    name: `Test ${id}`,
...
test("Pi ModelRuntime composes native, user-store, and extension-registered models through managed overrides", async () => {
  ...
  try {
    ...
    runtime.registerProvider("extension-registered", {
      ...
      models: [runtimeModel("frozen", 1_048_576, 524_288)],
      apiKey: "test-only",
    });
    runtime.registerProvider("native-test", {
      api: "openai-completions",
      baseUrl: "https://native.invalid",
      models: [runtimeModel("large", 1_000_000, 128_000)],
      apiKey: "test-only",
    });
    await runtime.refresh({ allowNetwork: false });
    const before = [
      runtime.getModel("native-test", "large"),
      userStoreModels[0] as unknown as ModelLike | undefined,
    ...
    assert.deepEqual(
      [runtime.getModel("native-test", "large"), runtime.getModel("extension-registered", "frozen")]
    ...
    assert.deepEqual(
      [{
        provider: "user-store",
        id: "large",
        contextWindow: readJson(paths.modelsPath).providers["user-store"].modelOverrides.large.contextWindow,
        maxTokens: readJson(paths.modelsPath).providers["user-store"].modelOverrides.large.maxTokens,
      }],
      [{ provider: "user-store", id: "large", contextWindow: 200_000, maxTokens: 32_768 }],
      "the managed override is the composition input for the persisted user-store catalog",
    );
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
